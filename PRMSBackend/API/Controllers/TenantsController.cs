...
namespace API.Controllers
{
    ...
    {
        readonly ITenantService _svc;

        public TenantsController(ITenantService svc)
        {
            _svc = svc;
        }
        ...
        {
            var data = await _svc.GetListAsync(ct) ?? Enumerable.Empty<TenantDto>();
            return Ok(APIResponse<IEnumerable<TenantDto>>.Success(data, "All data retrieved successfully."));
        }
        ...
        public async Task<IActionResult> Get(long id, CancellationToken ct = default)
        {
            ...
            return Ok(APIResponse<TenantDto>.Success(data!, "Data retrieved successfully."));
        }
        ...
        public async Task<IActionResult> Create([FromBody] TenantDto dto, CancellationToken ct = default)
        {
            if (dto == null)
        ...
        [ProducesResponseType(typeof(APIResponse<string>), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(APIResponse<string>), StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Update(long id, [FromBody] TenantDto dto, CancellationToken ct = default)
        {
